...
def resampled_seed_sos(kind: str, source_rate: int, target_rate: int, sos: Sos) -> Sos:
    """Map both power-complementary factors through the same all-pass warp.

    Substituting ``q -> (q-alpha)/(1-alpha*q)`` preserves the unit circle and
    therefore preserves the frozen pair's power complement exactly. Choosing
    alpha from the wire-rate ratio matches the low-frequency physical scale;
    the residual bilinear warp is negligible across the 24--32 kHz transition.
    """

    del kind
    ratio = target_rate / source_rate
    alpha = (ratio - 1.0) / (ratio + 1.0)
    mapped = []
    for b0, b1, b2, a1, a2 in strict_minimum_phase_sos(sos):
        def transform(c0: float, c1: float, c2: float) -> list[float]:
            return [
                c0 - alpha * c1 + alpha * alpha * c2,
                -2.0 * alpha * c0 + (1.0 + alpha * alpha) * c1 - 2.0 * alpha * c2,
                alpha * alpha * c0 - alpha * c1 + c2,
            ]

        numerator = transform(b0, b1, b2)
        denominator = transform(1.0, a1, a2)
        mapped.append([
            *(value / denominator[0] for value in numerator),
...
def profiles(wire_rate: int) -> tuple[Sos, Sos, StateSpace, StateSpace]:
    if wire_rate in FITTED_SOS:
        ...
        if wire_rate % 44_100 == 0:
            source_rate = 2_822_400
        elif wire_rate % 48_000 == 0:
            source_rate = 3_072_000
        else:
            raise ValueError(f"unsupported wire-rate family: {wire_rate}")
        if wire_rate <= source_rate:
            raise ValueError(f"no frozen profile fit for wire rate {wire_rate}")
        reconstruction_sos = resampled_seed_sos(
            "reconstruction",
            source_rate,
            wire_rate,
            FITTED_SOS[source_rate]["reconstruction"],
        )
        ultrasonic_sos = resampled_seed_sos(
            "ultrasonic",
            source_rate,
            wire_rate,
            FITTED_SOS[source_rate]["ultrasonic"],
        )
    return (
        reconstruction_sos,
        ultrasonic_sos,
...
def main() -> None:
    ...
    parser.add_argument(
        "--wire-rate",
        type=int,
        help="with --rust, emit one derived higher-rate profile pair",
    )
    args = parser.parse_args()
    if args.wire_rate is not None:
        if not args.rust:
            parser.error("--wire-rate requires --rust")
        _, _, reconstruction, ultrasonic = profiles(args.wire_rate)
        print("// Generated by tools/gen_beam_error_profiles.py --rust --wire-rate "
              f"{args.wire_rate}; do not hand edit.")
        print(f"// wire_rate={args.wire_rate}; sixth-order strict-minimum-phase linear-power fit")
        emit_profile(f"RECONSTRUCTION_{args.wire_rate}", reconstruction)
        emit_profile(f"ULTRASONIC_{args.wire_rate}", ultrasonic)
        return
    if args.rust:
        emit_rust()
    elif args.refit:
