// Modified by the Fozmo project on 2026-07-13.
// Changes conditionally compile FDK AAC support and reject AAC when disabled,
// preserving an ALAC-only build.

//! Audio encoding for AirPlay streaming.

use airplay_core::{AudioFormat, AudioCodec, error::Result};
#[cfg(test)]
...
pub fn create_encoder(format: AudioFormat) -> Result<Box<dyn AudioEncoder>> {
    match format.codec {
        ...
        AudioCodec::Aac => {
            #[cfg(feature = "aac")]
            {
                Ok(Box::new(AacEncoder::new(format)?))
            }
            #[cfg(not(feature = "aac"))]
            {
                Err(airplay_core::error::StreamingError::InvalidFormat(
                    "AAC support is disabled; this build is ALAC-only".to_string(),
                )
                .into())
            }
        }
...
mod tests {
    ...
    mod create_encoder {
        ...
        #[test]
        #[cfg(not(feature = "aac"))]
        fn rejects_aac_when_feature_is_disabled() {
            assert!(create_encoder(aac_format_44100()).is_err());
        }
